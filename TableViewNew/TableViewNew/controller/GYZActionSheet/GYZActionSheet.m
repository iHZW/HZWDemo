...
@property (strong, nonatomic) SelectLastLineBlock selectLastLineBlock;
...
//    [self addSubview:self.marginView];
    ...
    //布局子控件
    int cellCount = (int)itemTitles.count;
    self.contentVH = kCellH * (cellCount + isTitle)+50;
    if (self.contentVH > kSheetViewMaxH) {
        self.contentVH = kSheetViewMaxH;
        self.sheetView.tableView.scrollEnabled = YES;
    } else {
    ...
    self.footButton.layer.borderWidth = 0.5;
    self.footButton.layer.borderColor = [UIColor grayColor].CGColor;
    ...
    self.sheetView.frame = CGRectMake(0, sheetY, kScreenWidth, sheetH+kMargin);
...
//    }
    ...
    self.sheetView.cellTextStyle = NSTextStyleCenter;
    self.sheetView.tableView.scrollEnabled = YES;
...
- (void)setCellTextStyle:(NSCellTextStyle)cellTextStyle
{
    if (cellTextStyle)
    {
        self.sheetView.cellTextStyle = cellTextStyle;
    }
}


- (void)setShowFootView:(BOOL)showFootView
{
    self.sheetView.showFootView = showFootView;
}
...
- (void)didFinishSelectLastLine:(SelectLastLineBlock)block
{
    _selectLastLineBlock = block;
}
...
{
    if (_selectLastLineBlock)
    {
        _selectLastLineBlock();
    }
